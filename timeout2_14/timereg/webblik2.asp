...
<tr>
		...
		<a href="webblik2.asp?FM_parent=<%=oRec("id")%>&todolevel=<%=oRec("level")%>" class="<%=acls%>"><%=oRec("navn")%></a>&nbsp;<font class=lillegray>(<%=antalsubs%>)</font>
...
sub periodeForm%>
	<form method="post" action="webblik2.asp?menu=webblik&FM_usedatokri=1">
		...
		<%if joboversigt = 1 then%>
		&nbsp;&nbsp;
		<b>Risiko:</b>&nbsp;
		<select name="FM_risiko" id="FM_risiko" style="background-color:<%=selbgcol%>; font-size:9px; font-family:arial;">
		<option value="0" <%=riskCHK0%>>Alle</option>
		<option value="1" <%=riskCHK1%>>Lav</option>
		<option value="2" <%=riskCHK2%>>Mellem</option>
		<option value="3" <%=riskCHK3%>>Høj</option>
		</select>
		<%end if%>
...
if len(session("user")) = 0 then
	...
	<form method="post" action="webblik2.asp?func=opdaterjobliste">
	<tr bgcolor="#ffffff">
	...
	<tr height=30 bgcolor="#ffffe1">
		<td class=lille style="padding:3px;">&nbsp;</td>
		<td class=lille style="padding:3px;">Forv. afsl.</td>
		<td class=lille style="padding:3px;">Risiko</td>
		<td class=lille style="padding:3px;" align=right>Ress. tim.</td>
		<td class=lille style="padding:3px;" align=right>Forkalk.</td>
		<td class=lille style="padding:3px;" align=right>Forbrugt</td>
		<td class=lille style="padding:3px;">Rest. estimat</td>
		<td class=lille style="padding:3px;" align=right>Total</td>
		<td class=lille style="padding:3px;" align=right>Balance</td>
		<td class=lille style="padding:3px;" align=center>%</td>
		<td class=lille style="padding:3px;" align=right>Budget</td>
		<td class=lille style="padding:3px;">Udgifter</td>
		<td class=lille style="padding:3px;" align=right>Faktureret</td>
		<td class=lille style="padding:3px;" align=right>DB</td>
	</tr>
	<%
	...
	<tr bgcolor="<%=bgthis%>">
		...
		Response.write formatnumber(((totalforbrugt/timerTildelt)*100), 0) & "%"
		else%>
		&nbsp;
		<%end if%>
		</td>
	...
	|&nbsp;&nbsp;<a href="webblik2.asp?FM_parent=0" class=todo_lille>ToDo oversigt</a><br><br>
	<table cellspacing=0 cellpadding=0 border=0 width=400>
	<tr><form method=post action="webblik2.asp?FM_parent=<%=parent%>">
	...
	if cint(parent) = 0 then
			...
			<br><br><br><form method=post action="webblik2.asp?FM_parent=<%=parent%>">
			...
			%>
			<a href="webblik2.asp?FM_parent=<%=oRec3("id")%>&todolevel=<%=oRec3("level")%>&oldone=1" class="todo_lille">
	...
	<tr><td style="padding-top:0px;">
		...
		if len(trim(oRec("jobnavn"))) <> 0 then
			...
			<b><%=oRec("typenavn")%>:</b> <a href="javascript:popUp('milepale.asp?func=red&id=<%=oRec("mileid")%>&jid=<%=oRec("jid")%>&rdir=webblik','650','500','250','120');" target="_self" class=todo_mellem><%=oRec("navn")%></a>&nbsp;&nbsp;<a href="webblik2.asp?func=sletmilepal&milepalid=<%=oRec("id")%>" class=red>[x]</a><br>
			<%if len(oRec("beskrivelse")) <> 0 then%>
