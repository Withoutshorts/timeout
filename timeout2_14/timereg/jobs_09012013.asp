...
case "FN_getKundelisten"
            ...
            kundeArr(z) = "<option value='"&oRec("Kid")&"'>"&oRec("Kkundenavn")&"&nbsp;&nbsp;("&oRec("Kkundenr")&") " & anstxt &""& kans1 &"&nbsp;&nbsp;"& kans2 &"</option>"
			...
			oRec.close
              ...
              for z = 0 to z - 1
              '*** ÆØÅ **'
              call jq_format(kundeArr(z))
              kundeArr(z) = jq_formatTxt
	          
	          Response.Write kundeArr(z)
              next
...
if len(session("user")) = 0 then
	...
	&"Du vil samtidig slette <b>alle aktiviteter, timeregistreringer og<br> ressourcetimer</b> på dette job."
	...
	call sltque(slturlb,slttxtb,slturlalt,slttxtalt,210,290)
	...
	call sltque(slturlc,slttxtc,slturlalt,slttxtalt,580,90)
	...
	for t = 0 to UBOUND(jobids)
	    ...
	    if cint(fomrJa) = 1 then
                    ...
                    '*** nulstiller job (både job og akt) ****'
                    strSQLfor = "DELETE FROM fomr_rel WHERE for_jobid = "& jobids(t)  
    ...
    'Response.end
	Response.redirect "jobs.asp?menu=job&jobnr_sog="&request("jobnr_sog")&"&filt="&request("filt")&"&fm_kunde_sog="&request("fmkunde")&"&FM_kunde="&request("FM_kunde")
    ...
    'Response.end
		...
		if len(request("FM_navn")) = 0 OR len(trim(request("FM_jnr"))) = 0 OR len(request("FM_kunde")) = 0 OR _
		...
		Response.end
			...
			'Response.flush
					...
					Response.end
					 ...
					 'Response.Write "her oprettes"
					 	...
					 	if func = "dbopr" then  
                                ...
                                &" jobans_proc_1, jobans_proc_2, jobans_proc_3, jobans_proc_4, jobans_proc_5, restestimat, stade_tim_proc, virksomheds_proc, syncslutdato, altfakadr, preconditions_met) VALUES "_
							    &"('"& strNavn &"', "_
							    &"'"& strjnr &"', "_ 
							    &""& oRec("kid") &", "_  
							    &""& jo_gnsbelob &", "_ 
                                ...
                                &" "& virksomheds_proc &", "& syncslutdato &", "& altfakadr &", "& preconditions_met & ")")
							 ...
							 oRec5.close
                              ...
                              'end if 
								...
								'for a = 1 to 1
								    ...
								    call tilknytstamakt(a, intAktfavgp_use(a), trim(strAktFase_use(1)))
                            ...
                            &" syncslutdato = "& syncslutdato &", altfakadr = "& altfakadr &", preconditions_met = "& preconditions_met &""_
							&" WHERE id = "& id 
							...
							oConn.execute(strSQL)
								...
								'*** Opdaterer allerede tilknyttet aktiviteter   **********'
								    ...
								    call opdateraktliste(varJobId, aktids, aktnavn, akttimer, aktantalstk, aktfaser, aktbgr, aktpris, aktstatus, akttotpris, aktslet, aktslet_aids)
	            ...
	            if len(trim(request.form("FM_adviser_jobans"))) <> 0 then
				            ...
				            strSQL = "SELECT job.id AS jid, jobnavn, jobnr, jobans1, jobans2, job.beskrivelse, job_internbesk, m1.mnavn AS m1mnavn, m1.email AS m1email,"_
				            &" m2.mnavn AS m2mnavn, m2.email AS m2email, kkundenavn, kkundenr, m2.init AS m2init, m1.init AS m1init FROM job "_
				            ...
				            if request.servervariables("PATH_TRANSLATED") <> "C:\www\timeout_xp\wwwroot\ver2_1\timereg\jobs.asp" then
                                    ...
                                    if (jobans1 <> "0" AND isNULL(jobans1) <> true) OR (jobans2 <> "0" AND isNULL(jobans2) <> true) then
                                    
                                    'Sender notifikations mail
		                            Set Mailer = Server.CreateObject("SMTPsvg.Mailer")
		                            ' Sætter Charsettet til ISO-8859-1
		                            Mailer.CharSet = 2
		                            Mailer.FromName = "TimeOut Email Service | " & afsNavn 
                                    ...
                                    strBody = strBody &"<br><br><br><br><hr><br><br>Med venlig hilsen<br><i>" 
                    ...
                    'Response.end	
							...
							rdirLink = "jobs.asp?menu=job&func=red&id="&varJobId&"&jobnr_sog="&strJobsog&"&filt="&filt&"&fm_kunde="&strKundeId&"&fm_kunde_sog="&vmenukundefilt&"&showdiv="&showdiv&"&FM_tp_jobaktid="&tp_jobaktid&"&FM_mtype="&mtype
	...
	%>
	<!-- manual --->
    <div id="help" style="position:absolute; background-color:#ffffff; padding:1px 1px 0px 1px; width:170px; border:1px silver solid; border-bottom:0px; left:1020px; top:185px; visibility:visible; display:; z-index:9000000; overflow:hidden;">
	...
	<div id="sindhold" style="position:absolute; left:<%=sidedivLeft%>; top:<%=sidedivTop%>; visibility:<%=stdata_vzb%>; display:<%=stdata_dsp%>; z-index:50;">
	...
	<tr bgcolor="#5582D2">
		<td colspan=2 class="alt" valign="top" style="padding-top:4px;"><b><%=varbroedkrumme %></b></td>
	</tr>
	<%
	select case step
	case "1" 
	...
	jo_gnstpris = 0
    select case lto 
    case "epi", "epi_as", "epi_bar", "outz", "epi_ab"
    jo_gnsfaktor = 2
	case else
    jo_gnsfaktor = 1
    end select
    ...
    '** er der en SDSK priogruppe på kunde ****
    strSQL = "SELECT sdskpriogrp FROM kunder WHERE kid = "& strKundeId
    oRec.open strSQL, oConn, 3
    if not oRec.EOF then
    ...
    select case lto
    case "epi", "epi_as", "epi_bar", "intranet - local", "epi_ab"
	virksomheds_proc = 50
	syncslutdato = 1
    case else
    virksomheds_proc = 0
    ...
    &" diff_timer, diff_sum, jo_udgifter_ulev, jo_udgifter_intern, jo_bruttooms, restestimat, stade_tim_proc, virksomheds_proc, syncslutdato, lukkedato, altfakadr, preconditions_met "_
	...
	<tr bgcolor="#FFFFFF">
		...
		<td style="padding:10px 5px 10px 10px;" colspan="4">
        <div style="border:1px #ea0957 solid; padding:6px;">
        <img src="../ill/ikon_job_24.png" width="24" height="24" alt="" border="0">&nbsp;<font color=red size=2>*</font> <b>Jobnavn:</b> 
		...
		<input type="text" name="FM_navn" id="FM_navn" value="<%=strNavn%>" style="width:<%=twd %>px; color:<%=focol%>; font-size:12px; font-family:arial; font-style:italic;">
        <%if func = "red" then %>
        &nbsp;<font color=red size=2>*</font> Jobnr.: <input type="text" name="FM_jnr" id="FM_jnr" value="<%=strJobnr%>" style="width:80px; color:<%=focol%>; font-size:12px; font-family:arial; font-style:italic;">
        <%else %>
        <input type="hidden" name="FM_jnr" id="Text2" value="0">
        <%end if %>
        
        <br /><span style="font-size:10px; font-family:arial; color:#999999;">" (situationstegn) er ikke tilladt i jobnavn / jobnr. alfanumerisk</span>
        ...
        <br /><br />
                        ...
                        select case lto
                        case "synergi1", "intranet - local", "outz", "dencker", "essens"
	...
	<tr>
		...
		<%if cint(strStatus) = 3 then
					...
					<%if func = "red" then%>
					<br />Tilbudsnr: <input type="text" id="FM_tnr" name="FM_tnr" value="<%=strtilbudsnr%>" style="<%=tbStyle%>" size="20"> 
                    <%if strNexttilbudsnr <> strtilbudsnr then %> 
                    (næste ledige nummer: <%=strNexttilbudsnr %>)<br />
                    <%end if %>
					<%else%>
					...
					&nbsp;<input id="Text1" name="FM_sandsynlighed" value="<%=intSandsynlighed %>" type="text" style="width:30px; font-size:10px; font-family:arial;" /> % sandsynlighed for at vinde tilbud. &nbsp;
	...
	<tr bgcolor="#FFFFFF">
		...
		%>
		<select name="FM_serviceaft" id="FM_serviceaft" style="font-size:9px; width:380px;">
	...
	<tr>
		...
		<%if showaspopup <> "y" then %>
		<!-- intern note -->
        <div id="div_internnote" style="position:absolute; top:220px; left:20px; visibility:hidden; display:none; padding:3px; border:2px #8caae6 solid; background-color:#Effff3; width:450px; z-index:2000000;">
	...
	<tr>
		<td>&nbsp;</td>
		<td style="padding:0px 20px 5px 0px; width:140px;"><b>Rekvisitions Nr.:</b></td>
	...
	<!-- stam akt tilde SLUT --->
				   ...
				   <input id="jobfaktype" name="FM_jobfaktype" value="0" type="hidden" />   
					...
					<tr>
						<td style="">&nbsp;</td>
						<td colspan="2" style="padding-left:5px; padding-top:15px;" valign=top><h3>Jobansvarlig og jobejer
						...
						jbansTxt = "Jobmedansvarlig 1"
						jobansField = "jobans3, jobans_proc_3"
						case 4
						jbansImg = "<img src='../ill/blank.gif' width='24' height='24' alt='Jobejer' border='0'>"
						jbansTxt = "Jobmedansvarlig 2"
						jobansField = "jobans4, jobans_proc_4"
						case 5
						jbansImg = "<img src='../ill/blank.gif' width='24' height='24' alt='Jobejer' border='0'>"
						jbansTxt = "Jobmedansvarlig 3"
						...
						<td>
						<b><%=jbansTxt %>:</b></td><td>
						&nbsp;&nbsp;<select name="FM_jobans_<%=ja %>" id="FM_jobans_<%=ja %>" style="width:200px;">
						<option value="0">Ingen</option>
							...
							strSQL = "SELECT mnavn, mnr, mid, "& jobansField &" FROM medarbejdere "_
							&" LEFT JOIN job ON (job.id = "& id &") WHERE mansat <> 2"
							end if
						...
						</td><td><input id="FM_jobans_proc_<%=ja %>" name="FM_jobans_proc_<%=ja %>" value="<%=formatnumber(jobans_proc, 1) %>" type="text" style="width:40px;" /> %</td></tr>
                        ...
                        if level = 1 then 
                             if lto <> "epi" OR (lto = "epi" AND (thisMid = 6 OR thisMid = 11 OR thisMid = 59 OR thisMid = 1)) OR (lto = "epi_as" AND thisMid = 2) OR (lto = "epi_ab" AND thisMid = 2) OR (lto = "epi_bar" AND thisMid = 2) then%>
                        ...
                        <br /><br />
                        <input type="checkbox" value="1" name="FM_adviser_jobans" />Adviser valgte medarbejdere om at de er tilføjet som jobansvarlig / jobejer.
						 ...
						 <%'**** Projektgrupper *****%>
							...
							<tr>
								...
								<td colspan=2 style="padding-left:5px;">
                                 <h3><img src="../ill/projgrp.gif" />&nbsp;Projektgrupper <span style="font-size:9px; font-style:normal; font-weight:normal; line-height:11px;">- Hvem skal kunne registrere tid på dette job?</span></h3>
               ...
               <tr><td colspan=4 style="padding:20px 60px 40px 10px;">
                  ...
                  <%
                           if lto <> "execon" then%>
								...
								<input type="checkbox" name="FM_opdaterprojektgrupper" id="FM_opdaterprojektgrupper" value="1" <%=syncAktProjGrpCHK %>> <b>Opdater (Sync.) projektgrupper.</b> 
                                På alle tilhørende aktiviteter, så de følger
								de projektgrupper der er valgt på jobbet. 
								
                                <%end if%>

                                <%if func <> "red" then
                                 
                                    if lto = "jm" OR lto = "synergi1" then
                                    forvalgCHK = "CHECKED"
                                    else
                                    forvalgCHK = ""
                                    end if
                                 ...
                                 <br /><br />
                                 <div style="background-color:#FFFFe1; padding:1px;">
								<input type="checkbox" name="FM_forvalgt" id="FM_forvalgt" <%=forvalgCHK %> value="1"><b>Tilføj job til aktiv-jobliste.</b> 
                                Gør dette job aktivt på aktiv-joblisten for alle de medarbejdere der er med i de ovenstående valgte projektgrupper. <br />
                                <span style="color:#999999; line-height:14px;">Vælg "Tildel job" fra timereg. siden for at tilføje et job til aktiv-jobliste for en enkelt medarbejder i gruppen.</span>
                ...
                if func = "red" then
                    ...
                    &" LEFT JOIN fomr ON (fomr.id = for_fomr) WHERE for_jobid = "& id & " GROUP BY for_fomr"
                ...
                <tr bgcolor="#FFFFFF">
                                ...
                                <a href="#" class=vmenu id="a_tild_forr">+ Forretningsområder </a> (oprettes i kontrolpanelet)<br />
                            ...
                            <div id="div_tild_forr" style="visibility:hidden; display:none; background-color:#FFFFFF;">
                            <table cellpadding=0 cellspacing=0 border=0 width=100%>
                            <tr><td valign=top><br />
                            <b>Forretningsområder:</b><br />
                                ...
                                <select name="FM_fomr" id="FM_fomr" multiple="multiple" size="4" style="width:400px;">
				...
				<tr bgcolor="#FFFFFF">
                                ...
                                Tildel bla. prioitet, pre-konditioner, valuta, kundeadgang mm.
                    ...
                    <tr>
						...
						<td valign=top colspan=3>		
                                     ...
                                     <b>-2 = HR job</b> der men vises i HR mode på timereg. siden
				        ...
				        <td>&nbsp;</td>
                        <td valign=top style="padding-top:30px;" colspan=3>
                        <h3>Pre-konditioner opfyldt<br /><span style="font-size:9px; font-weight:normal;">
                   ...
                   <tr>
						<td>&nbsp;</td>
						<td colspan=3 style="padding:30px 5px 10px 0px;"><h3>Fakturerings valuta:</h3>
					    
					  
							<select name="FM_valuta" style="width:380px;">
...
<%if editok = 1 then '*** rediger rettigheder ok%>	
	...
	<tr bgcolor="#ffffff">
		<td colspan=4 align="right" style="padding:20px 40px 10px 10px; border-top:1px #cccccc solid;">
	...
	<%if (lto = "epi" OR lto = "epi_as" OR lto = "epi_bar" OR lto = "xintranet - local" OR lto = "epi_ab") AND func <> "red" AND step = 2 then 

    %>
    <div id="load" style="position:absolute; display:; visibility:visible; top:250px; left:80px; width:1000px; height:600px; background-color:#ffffff; border:10px #9ACD32 solid; padding:10px; z-index:100000000;">
    <table cellpadding=0 cellspacing=5 border=0 width=100%><tr><td>
	<img src="../ill/outzource_logo_200.gif" /><br />
	Indlæser aktiviteter, vent veligst. <br />Der kan gå op til 5 sek...
	...
	<!-------------------------------Sideindhold------------------------------------->
	<div id="sindhold" style="position:absolute; left:20px; top:112px; visibility:visible;">
	...
	<tr>
		<td colspan=2><b>Kontakt:</b>&nbsp;&nbsp;
		<select name="FM_kunde" id="FM_kunde" style="width:356px;" onchange="submit();">
    ...
    <input type="hidden" value="<%=hd_kpers %>" name="FM_hd_kpers" id="FM_hd_kpers" />
		...
		<select name="FM_kundekpers" id="FM_kundekpers" style="width:315px; font-size:9px;">
	...
	<select name="FM_medarb_jobans" id="FM_medarb_jobans" style="width:285px; background-color:#d6DFf5;">
    ...
    <select  name="FM_prjgrp" id="FM_prjgrp" style="width:408px;">
	...
	<tr bgcolor="#FFFFFF">
		<td colspan=2><br /><b>Søg på jobnr, jobnavn ell. kontakt:</b><br />
		<input type="text" name="jobnr_sog" id="jobnr_sog" value="<%=show_jobnr_sog%>" style="width:408px; background-color:#FFFFe1;">&nbsp;
		<br />
        (% = wildcard) Brug ">", "<" ell. "-" til at søge efter jobnr i et interval større, mindre eller mellem det angivede søgekriterie.<br />
        <br />
    ...
    <table border=0 cellspacing=0 cellpadding=0>
     ...
     <tr><td colspan=2>
         <b>Forretningsområder:</b> <%=left(strFomr_Gblnavn, 75) %><br />
                                ...
                                strSQLf = "SELECT id, navn FROM fomr WHERE id <> 0 ORDER BY navn"%>
                                <select name="FM_fomr" id="Select2" multiple="multiple" size="3" style="width:410px; font-size:9px;">
	...
	<tr>
		<td colspan=2><br /><br />
		<b>Vis kun job med slutdato i følgende periode:</b><br /> &nbsp;&nbsp;Ja:
		<input type="radio" name="usedatokri" value="j" <%=datoKriJa%>>&nbsp;Nej <input type="radio" name="usedatokri" value="n" <%=datoKriNej%>><font class=megetlillesort>(Sorterer efter slutdato)</font><br />
	...
	<tr bgcolor="#5582D2">
        ...
        <td class='alt' style="padding-right:10px;" valign=bottom><b>Forret.omr.</b></td>
        <td class='alt' valign=bottom style="width:100px;">&nbsp;&nbsp;&nbsp;<b>Realiseret %</b>&nbsp;</td>
   ...
   <tr><td style="font-size:9px; color:#CCCCCC;"><input type="radio" name="st_sel" value="1" id="st_cls_1" />Aktiv</td>
   <td style="font-size:9px; color:#CCCCCC;"><input type="radio" name="st_sel"  value="2" id="st_cls_2" />Passiv/Fak.</td></tr>  
   <tr><td style="font-size:9px; color:#CCCCCC;"><input type="radio" name="st_sel"  value="3" id="st_cls_3"/>Tilbud</td>
   <td style="font-size:9px; color:#CCCCCC;"><input type="radio" name="st_sel" value="4" id="st_cls_4" />Gennemsyn</td></tr> 
   <tr><td colspan=2 style="font-size:9px; color:#CCCCCC;"><input type="radio" name="st_sel" value="0" id="st_cls_0" />Lukket</td></tr> 
	...
	if cint(sogakt) = 1 then
        ...
        strSQL = strSQL &" FROM job AS j, kunder"
    end if
	...
	timerTotFakbarePajob = proaf 'oRec3("timerTotFaktimerpajob")
    ...
    <%
                        ...
                        if level = 1 then
                             if lto <> "epi" OR (lto = "epi" AND (thisMid = 6 OR thisMid = 11 OR thisMid = 59)) OR (lto = "epi_as" AND thisMid = 2) OR (lto = "epi_ab" AND thisMid = 2) OR (lto = "epi_bar" AND thisMid = 2) then 
	...
	&nbsp;</td>
		...
		'********* Aktiviteter ****
		if editok = 1 then%>
        &nbsp;<a href="#" onclick="Javascript:window.open('aktiv.asp?menu=job&jobid=<%=oRec("id")%>&jobnavn=<%=oRec("jobnavn")%>&rdir=job3&nomenu=1', '', 'width=1354,height=800,resizable=yes,scrollbars=yes')" class=vmenu>(<%=Antal%>) <img src="../ill/akt_24.png" alt="Se aktiviteter" border="0"></a>
		<%else%>
		<b>(<%=Antal%>)</b>
		<%end if
        ...
        'if visAkt <> 1 then
        %>
        <div style="font-size:9px; font-family:Arial; height:80px; width:120px; overflow:auto; color:#999999; padding:5px;"><%=strAktnavn %></div>
		...
		<div style="width:100px; border:1px #CCCCCC solid; height:20px;">
		<span style="width:<%=cint(left(projektcomplt, 3))%>px; background-color:<%=bgdiv%>; height:20px; padding:3px 0px 3px 3px;">
		...
		Real:  <%=formatnumber(proaf)%><br>
		<%restt = (budgettimertot - proaf)%>
	    -------------------<br />
        Bal. = <%=formatnumber(restt)%>
        =============
		</td>
        ...
        <input type="radio" class="FM_listestatus_4" name="FM_listestatus_<%=oRec("id")%>" value="4" id="FM_listestatus_4_<%=oRec("id")%>" <%=stCHK4%>/><span style="color:<%=stBgcol4%>; font-size:9px;">Gennemsyn</span><br />
	    ...
	    &"&FM_slut_mrd="&dtlink_slmd&"&FM_slut_aar="&dtlink_slaar
          ...
          <a href="timereg_akt_2006.asp?FM_kontakt=<%=oRec("kid")%>&FM_ignorer_projektgrupper=1&jobid=<%=oRec("id")%>&FM_sog_job_navn_nr=<%=oRec("jobnr")%>&usemrn=<%=session("mid")%>&showakt=1&fromsdsk=1" target="_blank" class=rmenu>Timeregistrering >> </a>
		...
		<%
			    ...
			    &" f.faktype, f.kurs, SUM(fd.aktpris) AS aktbel, brugfakdatolabel, labeldato FROM fakturaer f "_
			    ...
			    while not oRec3.EOF 
			        ...
			        <a href="erp_fak_godkendt_2007.asp?id=<%=oRec3("fid")%>&aftid=<%=oRec3("aftaleid")%>&jobid=<%=oRec3("jobid")%>" class="lgron" target="_blank"><b><%=oRec3("faknr") &"</b>"%></a></td><td align=right class=lille valign=top> <%=fakDato %> <br />
                    <%else%>
                    ...
                    belobGrundVal = -valBelobBeregnet
                        ...
                        if cDate(oRec3("fakdato")) < cDate("01-06-2010") AND (lto = "epi" OR lto = "epi_as" OR lto = "epi_bar" OR lto = "epi_ab") then
	...
	<tr>
		<td bgcolor="#d6dff5" style="padding-left:20;"><br>
		<font color="red"><b>Der blev ikke fundet nogen job der macthedede de valgte søgekriterier.</b><br>
		Vær opmærksom på om du har valgt:<br>
			<ul>
			<li>Et <u>Status filter.</u>
			<li>En <u>Periode.</u> 
			<li>Eller en bestemt <u>Kunde.</u></ul>
			Der betyder at der ikke blev fundet nogen job.</font>
		</td>
	</tr>
    ...
    <%if level = 1 then %>
     <tr style="background-color:#FFFFFF;"><td colspan=2>
        ...
        <br />
         <input id="Checkbox3" type="checkbox" name="FM_formr_opdater" value="1" /> Opdater alle job (og deres aktiviteter) på listen med ovenstående forretningsområder.
...
<tr> <input id="Hidden3" name="jids" value="<%=jids%>" type="hidden" />
   ...
   <input type=image src="../ill/export1.png" />
    ...
    <input type="hidden" value="1" name="eksDataStd" />
...
<input id="Hidden4" name="jids" value="<%=jids%>" type="hidden" />
   ...
   <input type=image src="../ill/printer3.png" />
	...
	call opretNyAB("jobs.asp?menu=job&func=opret&id=0&int=1", "Opret nyt job", otoppx, oleftpx, owdtpx) 
