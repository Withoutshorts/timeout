...
if len(session("user")) = 0 then
	...
	case "opdaterliste" 
            ...
            'Response.Write "her"
            'Response.flush
            Response.redirect "crmhistorik.asp?menu=crm&ketype=e&func=hist&selpkt=hist&medarb="&medarb&"&rdir="&rdir
            'Response.end
	...
	%>
	<!-------------------------------Sideindhold------------------------------------->
	<div id="sindhold" style="position:absolute; left:<%=divleft%>px; top:<%=divtop%>px; visibility:visible;">
	<%if showinwin <> "j" then%>
	<table cellspacing="0" cellpadding="0" border="0">
	<tr>
    <td valign="top"><br>
	...
	<td style="padding-top:5;" colspan="3"><br>Kunder:&nbsp;
    &nbsp;(<a href="kunder.asp?func=opret&ketype=k&hidemenu=1&showdiv=onthefly&rdir=1&menu=crm" target="_blank" class=vmenu>Opret ny kunde her..</a>)<br /><br />
	<select name="id" style="width:320px;">
		<%
		strSQL = "SELECT Kkundenavn, Kkundenr, Kid FROM kunder WHERE ketype <> 'xx' ORDER BY Kkundenavn" 'k
		oRec.open strSQL, oConn, 3
		while not oRec.EOF
		%>
	...
	<div id="andreakt" name="andreakt" style="position:absolute; left:250px; top:240px; background-color:#ffffff; width:100; z-index:100000; visibility:hidden; display:none; border:1px #000000 dashed; padding-left:5;">
	<a href="#" onClick="hideakt()" class='vmenu'>Luk vindue [X]</a><br><br>
				<%

                if len(trim(thisserialnb)) <> 0 then
                    thisserialnb = thisserialnb
                else
                    thisserialnb = -1
                end if
	...
	<tr>
		<td width=200><b>Kunder:</b>&nbsp; </td><td><select name="id" size="1" style="font-size : 11px; width:200px;" onchange="submit()";>
...
<tr>
		<td width=100><b>Kunde interesse niveau:</b>&nbsp; </td><td>
...
<tr>
			...
			<td colspan=2 style="padding-top:25px;"><b>Periode:</b><br>
	...
	<form method="post" action="crmhistorik.asp?func=opdaterliste">
	<input id="Hidden3" name="medarb" value="<%=medarb%>" type="hidden" />
    <input id="Hidden2" name="rdir" value="<%=rdir %>" type="hidden" />
	...
	<tr bgcolor="<%=bgcol %>">
		...
		<td valign="top" align=right style="padding:5px 15px 2px 0px;">
        
        <%=formatdatetime(oRec("crmdato"), 1)%></td>
