...
<%
  ...
  rem ******************************************
  	...
  	'strSQL = "SELECT emlremotehost, emlusername, emlpassword, mnavn FROM medarbejdere WHERE Mid = "& session("mid")
	'oRec.open strSQL, oConn, 3 
	
	'if not oRec.EOF then
	'strHost = "mail.outzource.dk" 'oRec("emlremotehost")
  	strNavn = "Søren Karlsen" 'oRec("mnavn") 'oRec("emlusername") 
  	strEmail = "sk@outzource.dk" 'oRec("emlpassword") 
	'end if
	...
	Set objSession = server.createobject("CDONTS.session")
 	'objSession.LogonSMTP = (""& strNavn &", "& strEmail &"")
	objSession.LogonSMTP "Søren Karlsen", "sk@outzource.dk"
	Set objInbox = objSession.Inbox
	
	Set colMessages = objInbox.Messages
	Response.write objInbox.Name & " - " & colMessages.Count & " mails " 
	
	'call  ShowMessageList ("mail.outzource.dk", "sk", "SKba200473")
	%>
