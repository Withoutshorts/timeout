...
if len(session("user")) = 0 then
	...
	<tr>
		<td valign="top" rowspan=3 width="10" style="border-top:1px #003399 solid;"><img src="../ill/tabel_top.gif" width="1" height="280" alt="" border="0"></td>
		<td height="10" colspan="4" style="border-top:1px #003399 solid;">&nbsp;</td>
		<td valign="top" align=right rowspan=3 style="border-top:1px #003399 solid;"><img src="../ill/tabel_top.gif" width="1" height="280" alt="" border="0"></td>
	</tr>
	<tr>
	...
	if func <> "red" then%>
	<tr>
		<td colspan=4 style="padding-left:10;" valign=top><br><b>Periode afgrænsning:</b><br>
	...
	strAktsubtotal = strAktsubtotal & "<tr>"_
		&"<td valign=top><img src=../ill/tabel_top.gif width=1 height=56 alt= border=0></td>"_
		&"<td valign=top><input type=hidden name='FM_aktid_"&x&"_"&a&"' value='"&thisaktid(x)&"'>"_
		&"<input type=hidden name='FM_hidden_timepristhis_"&x&"_"&a&"' id='FM_hidden_timepristhis_"&x&"_"&a&"' value='"&MedarbejderTimepris&"'>"
		...
		if func = "red" AND a <> 0 then
			...
			if usefastpris(x) <> 1 then 
					if a <> 0 then
						...
						strSQL2 = "SELECT sum(timer.timer) AS antaltimer FROM timer WHERE taktivitetid =" & thisaktid(x) &" AND tfaktim = 1 AND timepris = "&SQLBless(MedarbejderTimepris)&" AND (Tdato BETWEEN '" & stdatoKri &"' AND '"& slutdato &"')"   
	...
	strAktsubtotal = strAktsubtotal & "<input type=hidden name='FM_hidden_timerthis_"&x&"_"&a&"' id='FM_hidden_timerthis_"&x&"_"&a&"' value='"&hiddentimer&"'>"_
		&"<input type=checkbox name='FM_show_akt_"&x&"_"&a&"' id='FM_show_akt_"&x&"_"&a&"' value=1 "&chk&">&nbsp;"_
		&"<input type=text name='FM_timerthis_"&x&"_"&a&"' id='FM_timerthis_"&x&"_"&a&"' value='"&valueTimer&"' size=4 onkeyup='tjektimer("&x&","&a&"), setBeloebThis2("&x&","&a&")' onFocus='higlightakt("&x&","&n&","&nul&")'  style='!border: 1px; background-color: #FFFFFF; border-color: #86B5E4; border-style: solid;'></td>"_
		&"<td valign=top>&nbsp;&nbsp;<textarea cols=50 rows=3 name='FM_aktbesk_"&x&"_"&a&"' style='!border: 1px; background-color: #FFFFFF; border-color: #86B5E4; border-style: solid;'>"&thisaktnavn(x)&"</textarea>&nbsp;&nbsp;</td>"_
		&"<td valign=top>"_
		&" <input type=text name='FM_enhedspris_"&x&"_"&a&"' id='FM_enhedspris_"&x&"_"&a&"' onKeyup='tjektimer("&x&","&a&"), enhedspris("&x&","&a&"), setBeloebThis2("&x&","&a&")' onFocus='higlightakt("&x&","&n&","&nul&")' value='"&timepris&"' size=7"_ 
		...
		strAktsubtotal = strAktsubtotal & "<td valign=top>"_
		&"<input type=text name='FM_beloebthis_"&x&"_"&a&"' id='FM_beloebthis_"&x&"_"&a&"' value='"&thisbel&"' onkeyup='tjekBeloeb("&x&","&a&"), setBeloebTot2("&x&","&a&")' onFocus='higlightakt("&x&","&n&","&nul&")'"_
		&" size='8' style='!border: 1px; background-color: #FFFFFF; border-color: #86B5E4; border-style: solid;'></td>"_
		&" <td valign=top align=right><img src='../ill/tabel_top.gif' width=1 height=56 border=0></td>"_
	&"</tr>"
	...
	for x = 0 to x - 1 
		...
		sa = 0
					...
					<tr>
						<td style="border-left:1px #003399 solid;">&nbsp;</td>
						<td><br><font class=megetlillesort>Vis på fak. &nbsp;Fak | (v) Vent</td>
						<td><br>&nbsp;&nbsp;&nbsp;<b><%=thisaktnavn(x)%></b><font class=megetlillesort>&nbsp;&nbsp;&nbsp;Medarbejder(e)&nbsp;&nbsp;</td>
						<td><br><font class=megetlillesort>Timepris</td>
						<td><br><font class=megetlillesort>Beløb</td>
						<td style="border-right:1px #003399 solid;">&nbsp;</td>
					</tr>
					<%end if
					faktot = 0
					...
					while not oRec3.EOF
							...
							<tr>
								...
								<input type="text" name="FM_m_fak_<%=n%>_<%=x%>" id="FM_m_fak_<%=n%>_<%=x%>" onKeyup="tjektimer2('<%=x%>','<%=n%>'), updantaltimerprakt('<%=x%>','<%=n%>', 1)" onFocus="higlightakt('<%=x%>','<%=n%>','1')" value="<%=fak%>" size=2 style="!border: 1px; background-color: #FFFFFF; border-color: limegreen; border-style: solid; font-size:9px">&nbsp;
								...
								<input type="text" name="FM_mtimepris_<%=n%>_<%=x%>" id="FM_mtimepris_<%=n%>_<%=x%>" onKeyup="tjektimer3('<%=x%>','<%=n%>'), enhedsprismedarb('<%=x%>','<%=n%>')" onFocus="higlightakt('<%=x%>','<%=n%>','1')" value="<%=medarbejderTimepris%>" size=6 style="!border: 1px; background-color: #FFFFFF; border-color: #999999; border-style: solid; font-size:9px"></td>
								<td>
								<%
								...
								%>
								<input type="text" name="FM_mbelob_<%=n%>_<%=x%>" id="FM_mbelob_<%=n%>_<%=x%>" value="<%=SQLBlessDot(formatnumber(mbelob, 2))%>"  maxlength=0 onFocus="alertmedarbbelob('<%=x%>','<%=n%>')" onKeyup="alertmedarbbelob('<%=x%>','<%=n%>')" size=8 style="!border: 1px; background-color: #FFFFFF; border-color: #999999; border-style: solid; font-size:9px">
								</td>
								<td style="border-right:1px #003399 solid;">&nbsp;</td>
							</tr>
							<%
