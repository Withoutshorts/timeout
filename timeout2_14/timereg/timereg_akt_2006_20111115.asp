	...
	<%
	   ...
	   '**** Søgekriterier AJAX **'
        ...
        case "FN_showakt"        
           ...
           'job_akt_tregdatoer = request("job_akt_tregdatoer")
           
           'Response.Write request("job_iRowLoops")
           'Response.end
            ...
            'Response.Write request("job_aktids")
            'Response.Write "job_iRowLoops" & request("job_iRowLoops")
            'Response.end

              intEasyreg = request("FM_easyreg")

            'Response.Write "intEasyreg: " & intEasyreg
            'Response.end

              'if intEasyreg <> 1 then
            job_aktids = split(request("job_aktids"),",")
            'else
            'job_aktids = "0,1" 
            'job_aktids = split(job_aktids,",")
           ...
           if cint(intEasyreg) <> 1 then
                   ...
                   if lastFase <> lcase(thisFase) AND len(trim(thisFase)) <> 0 AND thisFase <> "nonexx99w" then 
			            ...
			            strAktiviteter = strAktiviteter & "<a id='"&lcase(thisFase)&"' name='"&lcase(thisFase)&"' href='#"&lcase(thisFase)&"' class='afase'> + "& tsa_txt_329 &": "& replace(thisFase, "_", " ")&"</a></td>"
			            ...
			            if request.Cookies("tsa_fase")(""&jobid&"_"&lcase(thisFase) &"") = "visible" then
                        tr_vzb = "visible"
			            tr_dsp = ""
                        else
                        tr_vzb = "hidden"
		                tr_dsp = "none"
                         ...
                         '*** Faser ***'
			            strAktiviteter = strAktiviteter & "<input name=""faseshow"" id='faseshow_"&lcase(thisFase)&"' type=""hidden"" value='"& tr_vzb &"' />"
			            strAktiviteter = strAktiviteter & "<input name=""faseshowid"" type=""hidden"" value='"&jobid&"_"&lcase(thisFase) &"' />"
                        '** end faser **'
            ...
            '**faser end
                                    ...
                                    strSQLakt = "SELECT j.id AS jid, navn AS aktnavn, tidslaas, tidslaas_st, tidslaas_sl, tidslaas_man, tidslaas_tir, "_
                                    &"tidslaas_ons, tidslaas_tor, tidslaas_fre, tidslaas_lor, tidslaas_son, k.kkundenavn, k.kkundenr, k.kid, j.jobnavn, j.jobnr, "_
                                    ...
                                    'if intEasyreg = 1 then
                                    'strSQLakt = strSQLakt & aktidsSQL &")"
                                    'else
                                    strSQLakt = strSQLakt & " WHERE a.id = "& job_aktids(r) & "" & strSQLAktDatoKri
                                    'end if
                                    ...
                                    'Response.end
                                            ...
                                            aktnavn = oRec3("aktnavn")
                                            call jq_format(aktnavn)
                                            aktnavn = jq_formatTxt

                                            job_tidslass_1 = oRec3("tidslaas_man")
                                            job_tidslass_2 = oRec3("tidslaas_tir")
                                            job_tidslass_3 = oRec3("tidslaas_ons")
                                            job_tidslass_4 = oRec3("tidslaas_tor")
                                            job_tidslass_5 = oRec3("tidslaas_fre")
                                            job_tidslass_6 = oRec3("tidslaas_lor")
                                            job_tidslass_7 = oRec3("tidslaas_son")
            
                                            job_tidslass = oRec3("tidslaas")
                                            job_tidslassSt = oRec3("tidslaas_st")
                                            job_tidslassSl = oRec3("tidslaas_sl")
            
                                          
                                            call jq_format(oRec3("kkundenavn"))
                                            job_kundenavn = jq_formatTxt

                                            job_kundenr = oRec3("kkundenr")
                                            job_kid = oRec3("kid")

                                            job_jobnavn = oRec3("jobnavn")
                                            job_jobnr = oRec3("jobnr")

                                            aktstartdato = oRec3("aktstartdato")
                                            aktslutdato = oRec3("aktslutdato")

                                            job_incidentid = oRec3("incidentid")
                                            job_fakturerbar = oRec3("fakturerbar")
                                            tfaktimThis = oRec3("fakturerbar") 'oRec3("tfaktim")

                                            job_jobans1 = oRec3("jobans1")
                                            job_jobans2 = oRec3("jobans2")

                                            job_bgr = oRec3("bgr")

                                            job_aktbudgettimer = oRec3("aktbudgettimer")
	                                        job_antalstk = oRec3("antalstk")

                                            job_abesk = oRec3("abesk")
                                        ...
                                        'strAktiviteter = strAktiviteter & "</tr></table>"
                                          ...
                                          strAktiviteter = strAktiviteter & "<tr class='td_"&lcase(job_akt_faser(r))&"' id='"& job_aktids(r) &"' style='visibility:"&tr_vzb&"; display:"&tr_dsp&";'>"
                                          ...
                                          '**job og akt. arrays **'
                                          strAktiviteter = strAktiviteter & "<input type='hidden' name='FM_jobid' id='FM_jobid_"& job_iRowLoops(r) &"' value='"& job_jid &"'>"
		                                  strAktiviteter = strAktiviteter & "<input type='hidden' name='FM_aktivitetid' id='FM_aktivitetid' value='"& job_aktids(r) &"'>" 
                                                            ...
                                                            strAktiviteter = strAktiviteter & "<span style='font-size:9px; color:#5582d2;'>"& left(job_kundenavn, 10) &"</span>"
		                                                       ...
		                                                       '*** jobnavn ****'
                                                                    ...
                                                                    '*** Fase ***'
                                                                    if len(trim(job_akt_faser(r))) <> 0 AND trim(job_akt_faser(r)) <> "nonexx99w" then
                                                                    strAktiviteter = strAktiviteter & "&nbsp;<span style='font-size:9px; color:yellowgreen;'>"& left(job_akt_faser(r), 20) &"</span>"
                                                                    end if 
                                                               
                                                                
                                                                    strAktiviteter = strAktiviteter & "&nbsp;<span href=""#"" class=""fjeasyregakt"" id='"& job_aktids(r) &"' style=""color:red; font-size:8px; cursor:hand;"">[X]</span>"
                                                           ...
                                                           if cint(intEasyreg) <> 1 then
                                                                    ...
                                                                    'if level = 1000 then
				                                                            ...
				                                                            if cint(aty_fakbar) = 1 OR job_fakturerbar = 5 then '** Viser tpriser på fakturerbare eller kørsel
				                                                                ...
				                                                                'else 'medarbtimepriser
				                                                                        ...
				                                                                        call alttimepris(job_aktids(r), jobid, usemrn, 0)  
                                                            ...
                                                            akt_timertot = 0
                                                            strSQLttot = "SELECT SUM(timer) AS timertot FROM timer WHERE taktivitetid = "& job_aktids(r) & " GROUP BY taktivitetid"
                                                            ...
                                                            strSQLtmnr = "SELECT SUM(timer) AS timermnr FROM timer WHERE taktivitetid = "& job_aktids(r) & " AND tmnr = "& usemrn &" GROUP BY taktivitetid"
				                                            ...
				                                            strAktiviteter = strAktiviteter & "<span style='font-family:Arial; font-size:8px; color:#5582d2;'> ("& formatnumber(akt_timerTot_medarb, 2) &")</span>"
		                                                                ...
		                                                                strAktiviteter = strAktiviteter & "<div id='div_aktbesk_"& job_aktids(r) &"' style='position:relative; visibility:visible; display:; left:0px; top:0px; width:200px; height:14px; overflow:hidden; z-index:200000;'>"
		                                                                ...
		                                                                strAktiviteter = strAktiviteter & "<a class='a_showhideaktbesk' id='aktbesk_"& job_aktids(r) &"' href='#'>"& job_abesk &"</a></td></tr></table></div>"
                                ...
                                for x = 1 to 7
                                    ...
                                    fmborcl = "1px #999999 solid"
				                            ...
				                            strSQLtim = "SELECT timer, timerKom, tfaktim, bopal, godkendtstatus, offentlig, sttid, sltid, destination FROM timer WHERE taktivitetid = "& job_aktids(r) &" AND tdato = '"& SQldatoX &"' AND tmnr = "& usemrn
                                            ...
                                            'tfaktimThis = oRec3("tfaktim")
                                            timerThis = timerThis + oRec3("timer")
                                            timerKomThis = timerKomThis & oRec3("timerkom") 
                                            offentlig = oRec3("offentlig")
                                            bopalThis = oRec3("bopal")
                                            destinationThis = oRec3("destination")
                                            godkendtstatus = oRec3("godkendtstatus")
                                          
                                                    if len(trim(oRec3("sttid"))) <> 0 then
		                                                if left(formatdatetime(oRec3("sttid"), 3), 5) <> "00:00" then
		                                                timer_sttid = left(formatdatetime(oRec3("sttid"), 3), 5)
		                                                else
		                                                timer_sttid = ""
		                                                end if
	                                                else
	                                                ...
	                                                if len(trim(oRec3("sltid"))) <> 0 then
		                                                if left(formatdatetime(oRec3("sltid"), 3), 5) <> "00:00" then
		                                                timer_sltid = left(formatdatetime(oRec3("sltid"), 3), 5)
		                                                else
		                                                timer_sltid = ""
		                                                end if
	                                                else
                                            ...
                                            'if cdbl(timerThis) <> -10 then			
								                    ...
								                    felt = felt & "<td valign='top' bgcolor='"&bgColor&"' style='border-bottom:1px #cccccc solid; border-right:1px #cccccc solid; padding-top:6px;' id='td_"&job_iRowLoops(r)&""&x&"'>"
								                     
                                                   
								                    
								
								                    if visTimerElTid <> 1 OR cint(tfaktimThis) = 5 OR cint(tfaktimThis) = 61 OR aty_pre <> 0 OR cint(intEasyreg) = 1 then
								                    timtype = "text"
								                    tidtype = "hidden"
								                    br = ""
								                    else
								                    ...
								                    'if lto <> "execon" then
								                    felt = felt &"<input class='dcls_"&x&"' type='"& timtype &"' name='FM_timer' id='FM_timer_"&job_iRowLoops(r)&"_"&x&"' value='"& timerThis &"' maxlength='"&maxl&"' style=""background-color:"&fmbgcol&"; border:"&fmborcl&"; height:16px; width:45px; font-family:arial; font-size:10px; line-height:12px;"" onkeyup=""doKeyDown()"" onfocus=""markerfelt('"&job_iRowLoops(r)&""&x&"','"&x&"'), showKMdailog('"&job_fakturerbar&"', '"&job_iRowLoops(r)&""&x&"', '"& bopalThis &"', '"& job_iRowLoops(r) &"', '"& job_kid &"')"";>"
								                    felt = felt &"<input type='hidden' name='FM_timer_opr' id='FM_timer_opr_"&job_iRowLoops(r)&"_"&x&"' value='"& timerThis &"'>"
								                    'else
								                    'felt = felt &"<input type='"& timtype &"' name='FM_timer' id='FM_timer' value='"& aktdata(job_iRowLoops(r), 1) &"' maxlength='"&maxl&"' style=""background-color:"&fmbgcol&"; border:"&fmborcl&"; height:16px; width:45px; font-family:arial; font-size:10px; line-height:12px;"" onfocus=""markerfelt('"&job_iRowLoops(r)&""&x&"','"&x&"')"";>"
								                    'end if
								                    ...
								                    felt = felt &"<input type='"& tidtype &"' name='FM_sttid' id='FM_sttid' value='"& timer_sttid &"' maxlength='"&maxl&"' style=""background-color:"&fmbgcol&"; height:16px; width:35px; font-family:arial; font-size:10px; line-height:12px; border:"&fmborcl&";"">"_
								                    &""& br &"<input type='"& tidtype &"' name='FM_sltid' id='FM_sltid' value='"& timer_sltid &"' maxlength='"&maxl&"' style=""background-color:"&fmbgcol&"; height:16px; width:35px; font-family:arial; font-size:10px; line-height:12px; border:"&fmborcl&";"">"
								
								                    if maxl <> 0 then
								                    felt = felt &"&nbsp;<a class=""a_showkom"" href=""#anc_s0"" name='anc_"&job_iRowLoops(r)&"_"&x&"' onClick=""expandkomm('"&job_iRowLoops(r)&"', '"&x&"')"">"& kommtrue &"</a>"
								                    end if
								                    ...
								                    &"<input class='d_kom_cls_"&x&"' type='hidden' name='FM_kom_"&job_iRowLoops(r)&""&x&"' id='FM_kom_"&job_iRowLoops(r)&""&x&"' value='"&timerKomThis&"'>"_
								                    &"<input type='hidden' name='FM_feltnr' id='FM_feltnr' value='"&job_iRowLoops(r)&""&x&"'>"
								                    ...
								                    felt = felt &"<input type='hidden' name='FM_off_"&job_iRowLoops(r)&""&x&"' id='FM_off_"&job_iRowLoops(r)&""&x&"' value='"&kommOnOff&"'>"
								                    felt = felt &"<input id='FM_bopal_"&job_iRowLoops(r)&""&x&"' name='FM_bopal_"&job_iRowLoops(r)&""&x&"' type=""hidden"" value="""& bopalThis &"""/>"
								                    felt = felt &"<input id='FM_destination_"&job_iRowLoops(r)&""&x&"' name='FM_destination_"&job_iRowLoops(r)&""&x&"' type=""hidden"" value="""& destinationThis &"""/>"
						                    ...
						                    <%'else
								              '          if instr(ugedagUdskrevet, weekday(tjekdag(x), 2)) = 0 then 
								                ...
								                '                end if
							'	
								                                'call akttyper2009Prop(aktdata(job_iRowLoops(r), 11)) 
								                 ...
								                 '               preVal = aty_pre
								                  ...
								                  '              end if
								                   ...
								                   '             erIndlast = 0
                                                                ...
                                                                ''call fakfarver(lastfakdato, tjekdag(1), tjekdag(x), erIndlast, usemrn, aktdata(job_iRowLoops(r), 11))
								
								                  '              felt = "<td valign=top bgcolor='"&bgColor&"' style=""border-bottom:1px #cccccc solid; border-right:1px #cccccc solid; padding-top:6px;"" id='td_"&job_iRowLoops(r)&""&x&"'>her"
								
								
								
								                   '             if visTimerElTid <> 1 OR aktdata(job_iRowLoops(r), 11) = 5 OR aktdata(job_iRowLoops(r), 11) = 61 OR aty_pre <> 0 OR cint(intEasyreg) = 1 then
								                   '             timtype = "text"
								                   '             tidtype = "hidden"
								                   '             br = ""
								                   '             else
								                   ...
								                   '             end if
								                                ...
								                                ''if lto <> "execon" then
								                   '             felt = felt &"<input class='dcls_"&x&"' type='"& timtype &"' name='FM_timer' id='FM_timer_"&job_iRowLoops(r)&"_"&x&"' value='"&preVal&"' maxlength='"&maxl&"' style=""background-color:"&fmbgcol&"; border:"&fmborcl&"; height:16px; width:45px; font-family:arial; font-size:10px; line-height:12px;"" onkeyup=""doKeyDown()""  onfocus=""markerfelt('"&job_iRowLoops(r)&""&x&"','"&x&"'), showKMdailog('"&aktdata(job_iRowLoops(r), 11)&"', '"&job_iRowLoops(r)&""&x&"', '0', '"& job_iRowLoops(r) &"', '"& topAdd &"')"";>"
								                   '             felt = felt &"<input type='hidden' name='FM_timer_opr' id='FM_timer_opr_"&job_iRowLoops(r)&"_"&x&"' value='"& aktdata(job_iRowLoops(r), 1) &"'>"
								
								                                ''else
								                                ''felt = felt &"<input type='"& timtype &"' name='FM_timer' id='FM_timer' value='"&preVal&"' maxlength='"&maxl&"' style=""background-color:"&fmbgcol&"; border:"&fmborcl&"; height:16px; width:45px; font-family:arial; font-size:10px; line-height:12px;"" onfocus=""markerfelt('"&job_iRowLoops(r)&""&x&"','"&x&"')"";>"
							                                    ''end if
								                   ...
								                   '             if maxl <> 0 then
								                   '             felt = felt &"&nbsp;<a class=""a_showkom"" href=""#anc_s0"" name='anc_"&job_iRowLoops(r)&"_"&x&"' onClick=""expandkomm('"&job_iRowLoops(r)&"', '"&x&"')""><font color='#999999'>+</font></a>"
								                   '             end if
								                   ...
								                   '             &"<input type='hidden' class='d_kom_cls_"&x&"' name='FM_kom_"&job_iRowLoops(r)&""&x&"' id='FM_kom_"&job_iRowLoops(r)&""&x&"' value=''>"_
								                   '             &"<input type='hidden' name='FM_feltnr' id='FM_feltnr' value='"&job_iRowLoops(r)&""&x&"'>"
								
								
								                   '             if len(aktdata(job_iRowLoops(r), 7) ) <> 0 then
								                   '             kommOnOff = aktdata(job_iRowLoops(r), 7) 
								                   '             else
								                   '             kommOnOff = 0
								                   '             end if
								                   ...
								                   '             felt = felt &"<input type='hidden' name='FM_off_"&job_iRowLoops(r)&""&x&"' id='FM_off_"&job_iRowLoops(r)&""&x&"' value='"&kommOnOff&"'>"
							                       '             felt = felt &"<input id='FM_bopal_"&job_iRowLoops(r)&""&x&"' name='FM_bopal_"&job_iRowLoops(r)&""&x&"' type=""hidden"" value=""0"" />"
								                   '             felt = felt &"<input id='FM_destination_"&job_iRowLoops(r)&""&x&"' name='FM_destination_"&job_iRowLoops(r)&""&x&"' type=""hidden"" value=""""/>"
								                   '             felt = felt &"</td>"
  ...
  '**************************************************************************************************
   ...
   <table cellpadding=0 cellspacing=0 border=0>
        ...
        <div id="div_tildel" style="position:absolute; left:779px; top:433px; width:210px; height:280px; visibility:hidden; display:none; z-index:100; border:1px #cccccc solid; padding:2px; background-color:#ffffff; overflow:auto;">
         ...
         Tildel job aktiv jobliste for:</b>
         </td>
   ...
   <a href="timereg_akt_2006.asp?showakt=1&FM_mnr=<%=usemrn%>&sortby=3&tildeliheledageSet=<%=tildeliheledageVal%>" class=rmenu><%=sortByUline3 %></a> / <a href="timereg_akt_2006.asp?showakt=1&FM_mnr=<%=usemrn%>&sortby=4&tildeliheledageSet=<%=tildeliheledageVal%>" class=rmenu><%=sortByUline4 %></a></span></h4>
	...
	for iRowLoop = 1 TO UBOUND(aktdata)
		   ...
		   if lastAktid <> aktdata(iRowLoop, 3) AND len(trim(aktdata(iRowLoop, 3))) <> 0 then
			...
			<%
                                ...
                                <!-- Henter aktiviteter --->
                                <table width=100%><tr><td style="padding:20px 20px 20px 20px;">Henter aktiviteter 2-4 sek...</td></tr></table>
            ...
            <%
                                ...
                                <tr><td>
                                                    ...
                                                    <input type="hidden" id="FM_job_lastfakdato_0" value="2001-1-1" />
                                                        <input type="hidden"  id="job_aktids_0" value="<%=strAktidsThisJob %>" />
                                                      <input type="hidden" name="fasenavn" id="job_fasenavne_0" value="<%=fsVal %>" />
 
                                                    <input type="hidden" name="iRowLoop" id="job_iRowLoop_0" value="<%=iRowLoopsThisJob %>" /> 
                                ...
                                <table width=100%><tr><td style="padding:20px 20px 20px 20px;">Henter Easyreg. aktiviteter (maks. 150) 5-10 sek...</td></tr></table>
    ...
    sonTimer = 0
     ...
     for x = 1 to 7
            ...
            strSQLtim = "SELECT SUM(timer) AS sumtimer, tjobnr, tjobnavn, Tknavn FROM timer WHERE ("& aty_sql_realhours &") AND tdato = '"& tjkTimerTotDato &"' AND tmnr = "& usemrn & " GROUP BY tjobnr"
            ...
            While not oRec3.EOF 
                ...
                select case x
                case 1
                manTimer = manTimer + oRec3("sumtimer")
                manTxt = manTxt & "<br><b>"& left(oRec3("Tknavn"), 12) &"</b><br>"& left(oRec3("tjobnavn"), 12) & ": "& oRec3("sumtimer")
                case 2
                tirTimer = tirTimer + oRec3("sumtimer")
                tirTxt = tirTxt & "<br><b>"& left(oRec3("Tknavn"), 12) &"</b><br>"& left(oRec3("tjobnavn"), 12) & ": "& oRec3("sumtimer")
                case 3
                onsTimer = onsTimer + oRec3("sumtimer")
                onsTxt = onsTxt & "<br><b>"& left(oRec3("Tknavn"), 12) &"</b><br>"& left(oRec3("tjobnavn"), 12) & ": "& oRec3("sumtimer")
                case 4
                torTimer = torTimer + oRec3("sumtimer")
                torTxt = torTxt & "<br><b>"& left(oRec3("Tknavn"), 12) &"</b><br>"& left(oRec3("tjobnavn"), 12) & ": "& oRec3("sumtimer")
                case 5
                freTimer = freTimer + oRec3("sumtimer")
                freTxt = freTxt & "<br><b>"& left(oRec3("Tknavn"), 12) &"</b><br>"& left(oRec3("tjobnavn"), 12) & ": "& oRec3("sumtimer")
                case 6
                lorTimer = lorTimer + oRec3("sumtimer")
                lorTxt = lorTxt & "<br><b>"& left(oRec3("Tknavn"), 12) &"</b><br>"& left(oRec3("tjobnavn"), 12) & ": "& oRec3("sumtimer")
                case 7
                sonTimer = sonTimer + oRec3("sumtimer")
                sonTxt = sonTxt & "<br><b>"& left(oRec3("Tknavn"), 12) &"</b><br>"& left(oRec3("tjobnavn"), 12) & ": "& oRec3("sumtimer")
                end select
    ...
    <td  align=right valign=top style="border-bottom:1px #CCCCCC solid;"><b><%=formatnumber(manTimer, 2)%></b>
    <span style="font-size:9px; color:#999999;"><%=manTxt %></span></td>
	<td  align=right valign=top style="border-bottom:1px #CCCCCC solid;"><b><%=formatnumber(tirTimer, 2)%></b>
    <span style="font-size:9px; color:#999999;"><%=tirTxt %></span></td>
	<td  align=right valign=top style="border-bottom:1px #CCCCCC solid;"><b><%=formatnumber(onsTimer, 2)%></b>
    <span style="font-size:9px; color:#999999;"><%=onsTxt %></span></td>
	<td  align=right valign=top style="border-bottom:1px #CCCCCC solid;"><b><%=formatnumber(torTimer, 2)%></b>
    <span style="font-size:9px; color:#999999;"><%=torTxt %></span></td>
	<td  align=right valign=top style="border-bottom:1px #CCCCCC solid;"><b><%=formatnumber(freTimer, 2)%></b>
    <span style="font-size:9px; color:#999999;"><%=freTxt %></span></td>
	<td  align=right valign=top style="border-bottom:1px #CCCCCC solid;"><b><%=formatnumber(lorTimer, 2)%></b>
    <span style="font-size:9px; color:#999999;"><%=lorTxt %></span></td>
	<td  align=right valign=top style="border-bottom:1px #CCCCCC solid;"><b><%=formatnumber(sonTimer, 2)%></b>
    <span style="font-size:9px; color:#999999;"><%=sonTxt %></span></td>
	<td  align=right valign=top style="border-bottom:1px #CCCCCC solid;">= <a href="<%=lnkUge%>" class=vmenu><%=formatnumber(totTimerWeek, 2)%></a></td>
	
    </tr>


    <%strSQLn = "SELECT medarbejdertype, normtimer_man, normtimer_tir, normtimer_ons, normtimer_tor, normtimer_fre, normtimer_lor, normtimer_son FROM medarbejdere AS m "_
    &" LEFT JOIN medarbejdertyper AS mt ON (mt.id = m.medarbejdertype) WHERE mid = "& usemrn 
    
    oRec3.open strSQLn, oConn, 3
    if not oRec3.EOF then

    normtimer_man = oRec3("normtimer_man")
    normtimer_tir = oRec3("normtimer_tir")
    normtimer_ons = oRec3("normtimer_ons")
    normtimer_tor = oRec3("normtimer_tor")
    normtimer_fre = oRec3("normtimer_fre")
    normtimer_lor = oRec3("normtimer_lor")
    normtimer_son = oRec3("normtimer_son")
    ...
    call fLonTimerPer(varTjDatoUS_man, 7, 20, usemrn)
...
<input type="hidden" name="lasttddaytype" id="lasttddaytype" value="0">
	   ...
	   <!---- Side indstillinger --->
	   <div id="pagesetadv" style="position:absolute; display:none; visibility:hidden; top:500px; left:125px; width:380px; height:155px; z-index:10000000000; background-color:#ffffff; border:10px #CCCCCC solid; padding:10px;">
         
        <table cellspacing=0 cellpadding=2 border="0" bgcolor="#FFFFFF" width="100%">
         <form action="timereg_akt_2006.asp?FM_use_me=<%=usemrn%>&showakt=1&jobid=<%=jobid%>&fromsdsk=<%=fromsdsk%>" method="post" name="timereg_indst" id="timereg_indst">
	   ...
	   <td align=right><a href="#" id="pagesetadv_close" class=red>[x]</a></td></tr>
		...
		<input type="checkbox" name="FM_ignJobogAktper" id="FM_ignJobogAktper" value="1" <%=ignJobogAktperCHK%>> <%=tsa_txt_286 %>
         ...
         <tr><td colspan=3>
		                <input id="Checkbox1" name="tildeliheledageSet" type="checkbox" value="1"/> <span style="color:#000000;"><%=replace(tsa_txt_277, "xxxx", "<br>") %> </span><br />&nbsp;
		                 </td></tr>
    ...
    if antalJobLinier > 20 OR antalaktlinier > 200 AND session("forste") = "j" then
...
t = visGuide
	if t = 11 AND session("forste") = "j" then
 


%>
<div id="div_velkommen" style="position:absolute; background-color:#FFFFFF; left:50px; top:80px; border:10px gold solid; padding:20px; width:700px; height:400px; z-index:100000000;">
<table><tr><td>
			
            <h4>Velkommen til den nye timereg. side <span style="font-size:10px;">10 okt. 2011 </span></h4>
	        Hermed en kort beskrivelse af de vigtigeste funktioner og ændringer på den nye timereg. side.<br /><br />
            <b>1) Register tid på flere job på en gang</b><br /> Det er nu muligt at registrere tid på mere end et job ad gangen. 
            Vælg flere job i søge filteret ved at holde [CTRL] nede mens du vælger.<br />
            For at gøre det nemt for dig at komme igang har vi forvalgt alle de job du har registreret timer på inden for den sidste uge.
            <br /><br />
            <img src="../ill/treg_2011_info.jpg" border=0/>

            <br />
            <b>2) Brug timeregistrerings siden som "task" liste.</b><br /> Du kan nu have alle de job du arbejder på netop nu, åbne på din "aktive job" liste.<br /><br />
            Desuden er det muligt for en projektleder at tilføje job til en medarbejders "aktive job liste", og dermed styre og have overblik over hvilke job den enkelte
            medarbejder er igang med netop nu. Det er muligt at sortere i joblisten ved at trække et job op eller ned.<br />
            <br />
            <b>3) Overblik og link til "Igangværende job"</b><br />
            Vi har tilføjet overblik over Igangværende job (og aktiviteter alt. efter licens) således at det bliver nemmere at overskue hvilke job der skal være færdige i den nærmeste fremtid.

            <br /><br />
            <a id="a_luk_velkommen" href="#">[Jeg har læst denne meddelelse, luk dette vindue]</a>
...
<%
if showakt = 1 AND browstype_client <> "ip" then
