...
if len(session("user")) = 0 then
			...
			'Response.flush
	            ...
	            if len(trim(request.form("FM_adviser_jobans"))) <> 0 then
				            ...
				            if request.servervariables("PATH_TRANSLATED") <> "C:\www\timeout_xp\wwwroot\ver2_1\timereg\jobs.asp" then
                                    ...
                                    'if lto = "outz" then
                                    'Response.write "M" & m &" isNULL(jobans1) & isNULL(jobans2)" &  isNULL(jobans1) &" "& isNULL(jobans2) & " afsEmail: "& afsEmail &"<br>"
                                    'end if    
    ...
    <%select case lto
    case "epi", "epi_cati", "epi_no", "epi_sta", "epi_se", "intranet - local"
    %>
      <!-- Popup MSG -->
    ...
    select case lto 
    case "epi", "epi_no", "epi_sta", "outz", "epi_ab", "epi_cati"
    jo_gnsfaktor = 2
	case else
    jo_gnsfaktor = 1
    end select
    ...
    &" syncslutdato, lukkedato, altfakadr, preconditions_met, laasmedtpbudget "_
	...
	<tr bgcolor="#FFFFFF">
		...
		<input type="text" name="FM_navn" id="FM_navn" value="<%=strNavn%>" style="width:<%=twd %>px; color:<%=focol%>; font-style:italic;">
        <%if func = "red" then %>
        &nbsp;<font color=red size=2>*</font> Jobnr.: <input type="text" name="FM_jnr" id="FM_jnr" value="<%=strJobnr%>" style="width:80px; color:<%=focol%>; font-style:italic;">
        <%else %>
	...
	<tr bgcolor="#FFFFFF">
		...
		%>
		<select name="FM_serviceaft" id="FM_serviceaft" style="width:380px;">
	...
	<!-- stam akt tilde SLUT --->
				   ...
				   <input id="jobfaktype" name="FM_jobfaktype" value="0" type="hidden" />   
                            ...
                            select case lto 
                            case "epi", "epi_ab", "epi_no", "epi_sta", "intranet - local"
					...
					<table width="100%">
						...
						</td><td><input id="FM_salgsans_proc_<%=sa %>" name="FM_salgsans_proc_<%=sa %>" value="<%=formatnumber(salgsans_proc, 1) %>" type="text" style="width:40px;" /> %</td></tr>
                        ...
                        if level = 1 then 
                             if lto <> "epi" OR (lto = "epi" AND (thisMid = 6 OR thisMid = 11 OR thisMid = 59 OR thisMid = 1 OR thisMid = 1720)) OR (lto = "epi_no" AND thisMid = 2) OR (lto = "epi_ab" AND thisMid = 2) OR (lto = "epi_sta" AND thisMid = 2) OR (lto = "epi_cati" AND thisMid = 2) then%>
               ...
               <tr><td colspan=4 style="padding:20px 60px 40px 10px;">
                ...
                <tr bgcolor="#FFFFFF">
                                ...
                                <a href="#" class=vmenu id="a_tild_forr">+ Forretningsområder </a> <span style="color:#999999;"> (Oprettes i kontrolpanelet) </span><br />
                                Forretningsområder bruges bl.a. til at se tidsforbrug på tværs af kunder og job, og til at se hvilke slags opgaver man bruger sin tid på.<br /><br />
                            ...
                            <table cellpadding=0 cellspacing=0 border=0 width=100%>
                            <tr><td valign=top>
                            <b>Forretningsområder:</b>                               
                                <%
                                
                                
                                strSQLf = "SELECT id, navn FROM fomr WHERE id <> 0 ORDER BY navn"%>
                                <select name="FM_fomr" id="FM_fomr" multiple="multiple" size="6" style="width:400px;">
				...
				<tr bgcolor="#FFFFFF">
                    ...
                    <tr>
						...
						<td valign=top colspan=3>		
									 ...
									 <br /><br />
                                     -1 / -2 medfører enkel visning af aktivitetslinjer på timereg. siden, dvs. der bliver ikke vist tidsforbrug, start og slutdatoer mv. på aktiviteterne. <br /><br />&nbsp;
...
<%if editok = 1 then '*** rediger rettigheder ok%>	
	...
	<%if (lto = "epi" OR lto = "epi_no" OR lto = "epi_sta" OR lto = "xintranet - local" OR lto = "epi_ab" OR lto = "epi_cati") AND func <> "red" AND step = 2 then 
	...
	<!-------------------------------Sideindhold------------------------------------->
	<div id="sindhold" style="position:absolute; left:20px; top:172px; visibility:visible;">
    ...
    <table border=0 cellspacing=0 cellpadding=0>
     ...
     <tr><td colspan=2>
         <b>Forretningsområder:</b> <%=left(strFomr_Gblnavn, 75) %><br />
                                ...
                                strSQLf = "SELECT id, navn FROM fomr WHERE id <> 0 ORDER BY navn"%>
                                <select name="FM_fomr" id="Select2" multiple="multiple" size="3" style="width:410px;">
   ...
   <tr><td style="font-size:9px; color:#CCCCCC;"><input type="radio" name="st_sel" value="0" id="st_cls_0" />Lukket</td></tr> 
    ...
    <%
                        ...
                        if level = 1 then
                             if lto <> "epi" OR (lto = "epi" AND (thisMid = 6 OR thisMid = 11 OR thisMid = 59 OR thisMid = 1720)) OR (lto = "epi_cati" AND (thisMid = 6 OR thisMid = 11 OR thisMid = 59)) OR (lto = "epi_no" AND thisMid = 2) OR (lto = "epi_ab" AND thisMid = 2) OR (lto = "epi_sta" AND thisMid = 2) then 
		...
		<%
			    ...
			    while not oRec3.EOF 
                    ...
                    belobGrundVal = -valBelobBeregnet
                        ...
                        if cDate(oRec3("fakdato")) < cDate("01-06-2010") AND (lto = "epi" OR lto = "epi_no" OR lto = "epi_sta" OR lto = "epi_ab" OR lto = "epi_cati") then
